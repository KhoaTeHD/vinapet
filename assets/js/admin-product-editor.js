...
jQuery(document).ready(function ($) {
  ...
  function updateSEOPreview() {
    ...
    const desc =
      $("#seo_description").val() || "Mô tả sản phẩm sẽ hiển thị ở đây...";
  ...
  $("#save-meta").on("click", function () {
    ...
    $.ajax({
      ...
      data: {
        ...
        custom_image_1: $('#custom_image_1').val(),
        custom_image_2: $('#custom_image_2').val(),
        custom_image_3: $('#custom_image_3').val(),
        custom_image_4: $('#custom_image_4').val(),
      },
  ...
  $("#delete-meta").on("click", function () {
    if (
      !confirm(
        "Bạn chắc chắn muốn xóa tất cả tùy chỉnh? Sẽ quay về dùng dữ liệu từ ERP."
      )
    ) {
      return;
    }
  ...
  // Upload Custom Images (4 ảnh)
  let customImageFrames = {};

  $(".upload-custom-image-btn").on("click", function (e) {
    e.preventDefault();

    const index = $(this).data("index");

    if (customImageFrames[index]) {
      customImageFrames[index].open();
      return;
    }

    customImageFrames[index] = wp.media({
      title: "Chọn hình ảnh " + index,
      button: {
        text: "Sử dụng ảnh này",
      },
      multiple: false,
    ...
    customImageFrames[index].on("select", function () {
      const attachment = customImageFrames[index]
        .state()
        .get("selection")
        .first()
        .toJSON();
      $("#custom_image_" + index).val(attachment.url);
      $("#preview-custom-image-" + index).html(
        '<img src="' +
          attachment.url +
          '" style="max-width: 100%; height: auto; border-radius: 5px;">'
      );
      $('.remove-custom-image-btn[data-index="' + index + '"]').show();
    });
  ...
  $(".remove-custom-image-btn").on("click", function (e) {
    e.preventDefault();
    const index = $(this).data("index");

    $("#custom_image_" + index).val("");
    $("#preview-custom-image-" + index).html(
