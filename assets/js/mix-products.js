(function($) {
    $(document).ready(function() {
        ...
        $('#add-product-badge').on('click', function() {
            if (!mixData.product2) {
                showMessage('Vui lòng chọn sản phẩm thứ 2 trước khi thêm sản phẩm thứ 3.', 'warning');
                return;
            }
            showThirdProduct();
        });
        ...
        function showSecondProductContent(name, description) {
            $('.secondary-product .product-title, .secondary-product .product-description').remove();
        ...
        function showThirdProductContent(name, description) {
            $('.third-product .product-title, .third-product .product-description').remove();
        ...
        function redistributePercentages(changedProduct = null) {
            ...
            if (changedProduct) {
                ...
                const otherProducts = activeProducts.filter(p => p !== `product${changedProduct}`);
                ...
                if (otherProducts.length === 1) {
                    ...
                    const currentTotal = mixData[otherProducts[0]].percentage + mixData[otherProducts[1]].percentage;
                    if (currentTotal > 0) {
                        const ratio1 = mixData[otherProducts[0]].percentage / currentTotal;
                        const ratio2 = mixData[otherProducts[1]].percentage / currentTotal;
                        
                        mixData[otherProducts[0]].percentage = Math.round(remaining * ratio1);
                        mixData[otherProducts[1]].percentage = remaining - mixData[otherProducts[0]].percentage;
                    } else {
                ...
                activeProducts.forEach((product, index) => {
                    mixData[product].percentage = equalPercentage + (index < remainder ? 1 : 0);
                });
            }
            ...
            const total = activeProducts.reduce((sum, product) => sum + mixData[product].percentage, 0);
            if (total !== 100) {
                ...
                if (mixData[activeProducts[0]].percentage < 10) {
                    ...
                    const excess = 10 - (100 - total + mixData[activeProducts[0]].percentage);
                ...
                if (mixData[activeProducts[0]].percentage > 90) {
                    ...
                    const excess = (100 - total + mixData[activeProducts[0]].percentage) - 90;
                    if (activeProducts.length > 1) {
                        mixData[activeProducts[1]].percentage += excess;
                    }
                }
        ...
        function updateFooterSummary() {
            ...
            const productCountText = activeProducts.length === 2 ? 'Mix 2 loại sản phẩm' : 'Mix 3 loại sản phẩm';
        ...
        $('#next-step-button').on('click', function(e) {
            ...
            const totalPercentage = activeProducts.reduce((sum, product) => sum + mixData[product].percentage, 0);
            if (Math.abs(totalPercentage - 100) > 1) {
                showMessage('Tổng tỷ lệ phải bằng 100%. Vui lòng điều chỉnh lại.', 'error');
                return;
            }
        ...
        function showMessage(message, type = 'info') {
            ...
            const messageClass = type === 'success' ? 'success' : type === 'error' ? 'error' : type === 'warning' ? 'warning' : 'info';
        ...
        window.debugPercentages = function() {
            ...
            console.log('Percentages:', activeProducts.map(p => `${p}: ${mixData[p].percentage}%`));
        ...
        console.log('Mix Products page initialized with 3-product support and percentage sliders');
    });
})(jQuery);