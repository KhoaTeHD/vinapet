...
class ERP_API_Client {
    ...
    const ENDPOINTS = [
        ...
        // Customer endpoints
        'create_customer'   => 'resource/Customer',
        'get_customers'     => 'resource/Customer',
        'update_customer'   => 'resource/Customer',
    ...
    public function get_customers($params = []) {
        ...
        $cache_key = 'erp_customers_' . md5(serialize($params));
        $customers = get_transient($cache_key);

        if (false === $customers) {
            $endpoint = $this->get_endpoint('get_customers');
            $response = $this->make_request('GET', $endpoint, $params);
            ...
            if (!is_wp_error($response)) {
                ...
                $cache_time = $this->get_cache_time('get_customers');
                set_transient($cache_key, $customers, $cache_time);
            } else {
                return false;
            }
        }
