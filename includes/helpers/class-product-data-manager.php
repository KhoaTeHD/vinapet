...
class Product_Data_Manager
{
        ...
        if ($this->erp_client && $this->erp_client->is_configured()) {
            try {
                ...
                if ($erp_response !== false && is_array($erp_response)) {
                    ...
                    // ===== GỌI API GIÁ VÀ GÁN VÀO pricing_rules =====
                    $products_data = $this->attach_pricing_rules($products_data);
    ...
    /**
     * Gọi API lấy giá cho từng sản phẩm và gán vào pricing_rules
     * @param array $products - Danh sách sản phẩm
     * @return array - Danh sách sản phẩm đã có pricing_rules
     */
    private function attach_pricing_rules($products)
    {
        ...
        // Kiểm tra ERP client
        if (!$this->erp_client || !$this->erp_client->is_configured()) {
            error_log('VinaPet: ERP Client not configured for pricing rules');
            return $products;
        }

        foreach ($products as &$product) {
            // Lấy mã sản phẩm
            $item_code = $product['Ma_SP'] ??
                $product['ProductID'] ??
                $product['item_code'] ?? '';
            ...
            // Gọi API lấy giá
            try {
                $price_response = $this->erp_client->get_product_price_detail($item_code);

                if ($price_response !== false && isset($price_response[1])) {
                    // Lưu pricing tiers vào trường pricing_rules
                    $product['pricing_rules'] = $price_response[1];
                } else {
                    $product['pricing_rules'] = [];
                }
            } catch (Exception $e) {
                error_log("VinaPet Pricing Error for {$item_code}: " . $e->getMessage());
                $product['pricing_rules'] = [];
            }
        }
